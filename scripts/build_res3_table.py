#!/usr/bin/env python
"""Build a new comparison table for the res3 delivery package."""
...
def main() -> None:
    output_dir = Path("outputs_cmp_110_res3opt")
    ...
    deep_fc = json.loads(
        Path("outputs_cmp_110_seed66/deep_hierarchical_fc_c2r_tune1/metrics.json").read_text(encoding="utf-8")
    )
    deep_rcnn_opt = json.loads(
        Path("outputs_cmp_110_seed66/deep_fusion_sweep/rcnn_crcnn.json").read_text(encoding="utf-8")
    )
    ...
    add_row(rows, "DL", "fc", "flat", "deep_hierarchical_fc_c2r_tune1", deep_fc["test"]["fine_flat"])
    add_row(rows, "DL", "fc", "hier_fusion", "deep_hierarchical_fc_c2r_tune1", deep_fc["test"]["fine_hier"])
    add_row(rows, "DL", "rcnn", "flat", "deep_hierarchical_rcnn_c2r", deep_rcnn_opt["base"]["test"])
    ...
    summary = {
        ...
        "note": "ML uses weighted SVM fusion; DL uses FC(c2r_tune1) and RCNN weighted+constraint sweep.",
    }
    (output_dir / "summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")

    print("[Done] res3 table generated")
